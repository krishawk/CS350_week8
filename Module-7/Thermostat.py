...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
       ...
       # OFF state → all lights off
        if self.state == self.OFF:
          pass

   # HEAT state → turn on heat light if temp is below setPoint
        elif self.state == self.HEAT:
         if self.currentTemp < self.setPoint:
          self.heatLight.on()
         else:
          self.heatLight.off()

# COOL state → turn on cool light if temp is above setPoint
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"Temp: {self.getFahrenheit()}F"
            ...
            if((counter % 30) == 0):
                self.Port.write(self.setupSerialOutput().encode())
                counter = 1
            else:
                counter = counter + 1
            sleep(1)
...
##
greenButton.when_pressed = tsm.cycle
